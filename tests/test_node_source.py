...
class TestMongodbClusterToNode:
    def test_output(self):
        cluster = _make_resource(name="mongodb-prod", id="mongodb-1", status=2)
        node = node_source.mongodb_cluster_to_node(cluster, "folder-1")

        assert node["resource_type"] == "managed-mongodb"
        assert node["status"] == "RUNNING"
