...
class TestStopComputeInstance:
    ...
    def test_custom_timeout_is_passed(self, mock_sdk, mocker):
        ...
        stop.stop_compute_instance(mock_sdk, "inst-id", timeout=600)
...
class TestStopK8sCluster:
    ...
    def test_custom_timeout_is_passed(self, mock_sdk, mocker):
        ...
        stop.stop_k8s_cluster(mock_sdk, "cluster-id", timeout=900)
...
class TestStopMongodbCluster:
    def test_already_stopped_skips(self, mock_sdk):
        svc = MagicMock()
        svc.Get.return_value = MagicMock(status=MONGODB_STOPPED)
        mock_sdk.client.return_value = svc

        stop.stop_mongodb_cluster(mock_sdk, "cluster-id")
    ...
    def test_stops_starting_cluster(self, mock_sdk, mocker):
        svc = MagicMock()
        svc.Get.return_value = MagicMock(status=MONGODB_STARTING)
        svc.Stop.return_value = MagicMock(id="op-2")
        mock_sdk.client.return_value = svc
        mocker.patch("stop.wait_for_operation")
...
class TestStopOpensearchCluster:
    def test_already_stopped_skips(self, mock_sdk):
        svc = MagicMock()
    ...
    def test_stops_starting_cluster(self, mock_sdk, mocker):
        svc = MagicMock()
        svc.Get.return_value = MagicMock(status=OPENSEARCH_STARTING)
        svc.Stop.return_value = MagicMock(id="op-2")
        mock_sdk.client.return_value = svc
        mocker.patch("stop.wait_for_operation")
...
class TestStopYdbDatabase:
    ...
    def test_custom_timeout_is_passed(self, mock_sdk, mocker):
        ...
        stop.stop_ydb_database(mock_sdk, "db-id", timeout=900)
